...
def main():
    ...
    processor = data_processor.NERProcessor()
    ...
    model = modeling.LaSAForNER(config)
    model.to(device)
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids,
                                   all_chunk_lens, all_label_ids,all_label_mask,all_valid_ids)
        if args.local_rank == -1:
        ...
        if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
            ...
            if args.do_random:
                all_chunk_lens = torch.tensor([do_random_chunk(f.chunks_len) for f in eval_features], dtype=torch.long)
            else:
            ...
            test_features = processor.get_test_features_from_examples(data_dir=args.data_dir, bert_type=args.bert_type,
                                                                      max_chunk_number=args.max_chunk_number,
                                                                      ngram_path=args.dict_dir,max_seq_len=args.max_seq_length)
            logger.info("  Num test examples = %d", len(eval_features))
            logger.info("  Batch size = %d", args.eval_batch_size)
            ...
            if args.do_random:
                all_chunk_lens = torch.tensor([do_random_chunk(f.chunks_len) for f in test_features], dtype=torch.long)
            else:
        ...
        for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                label_ids,label_mask,valid_ids = batch
                loss = model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                             chunk_lens=chunk_lens, label_ids=label_ids,label_mask=label_mask,valid_ids=valid_ids)[0]
                if n_gpu > 1:
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                ...
                with open(output_logits_file, "a") as writer:
                    writer.write(
                    ...
                    for batch in tqdm(eval_dataloader, desc="Evaluating"):
                        ...
                        label_ids, label_mask, valid_ids = batch
                        with torch.no_grad():
                            loss,logits,labels_id = model(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids,
                                  chunk_lens=chunk_lens, label_ids=label_ids, label_mask=label_mask,
                                  valid_ids=valid_ids)
                        logits = torch.argmax(F.log_softmax(logits, dim=1), dim=1)
                        logits = logits.detach().cpu().numpy()
                        labels_id = labels_id.detach().cpu().numpy()
                        for label_id in labels_id :
                            real_labels.append(label_map[label_id])
                        for logit in logits :
                            pred_labels.append(label_map[logit])
                ...
                logger.info("  Batch size = %d", args.eval_batch_size)
                # print('# model parameters:', sum(param.numel() for param in predict_model.parameters()))
                output_logits_file = os.path.join(args.output_dir, str(epoch) + "_test_logits_results.tsv")
                pred_labels = []
                real_labels = []
                label_map = {i: label for i, label in enumerate(label_list)}
                with open(output_logits_file, "a") as writer:
                    ...
                    for batch in tqdm(test_dataloader, desc="Evaluating"):
                        ...
                        label_ids, label_mask, valid_ids = batch
                        with torch.no_grad():
                            loss, logits, labels_id = model(input_ids=input_ids, attention_mask=attention_mask,
                                                            token_type_ids=token_type_ids,
                                                            chunk_lens=chunk_lens, label_ids=label_ids,
